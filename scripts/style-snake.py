"""Frame the rolling 180-day animation with its actual dates and contribution totals."""
...
def main():
    ...
    if metadata["days"] != 180:
        raise ValueError("The recent contribution card requires 180 calendar days")
    ...
    ET.SubElement(card, tag("title"), {"id": "title"}).text = "J.Tide 最近 180 天的 GitHub 贡献"
    ET.SubElement(card, tag("desc"), {"id": "description"}).text = (
        f"{metadata['start']} 至 {metadata['end']}，{metadata['contributions']} 次贡献，"
        f"{metadata['activeDays']} 个活跃日。按真实贡献日期生成，每日更新两次。"
    )
    ...
    text(40, 105, "最近 180 天", 28, "#edf3f8", "600")
